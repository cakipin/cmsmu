...
import { A as record, C as array, D as object, E as number, M as union, N as unknown, O as preprocess, S as any, T as date, j as string, k as promise, lt as s, w as boolean, x as _function } from "./server__3e0ELtE.mjs";
...
//#endregion
//#region node_modules/@astrojs/rss/dist/schema.js
var rssSchema = object({
...
//#endregion
//#region node_modules/@astrojs/rss/dist/util.js
function createCanonicalURL(url, trailingSlash, base) {
	let pathname = url.replace(/\/index.html$/, "");
	if (!getUrlExtension(url)) pathname = pathname.replace(/\/*$/, "/");
	pathname = pathname.replace(/\/+/g, "/");
	const canonicalUrl = new URL(pathname, base).href;
	if (trailingSlash === false) return canonicalUrl.replace(/\/*$/, "");
	return canonicalUrl;
}
...
function getUrlExtension(url) {
	...
	return lastDot > url.lastIndexOf("/") ? url.slice(lastDot + 1) : "";
}
//#endregion
//#region node_modules/@astrojs/rss/dist/index.js
var globResultValidator = record(string(), _function({
	input: [],
	output: promise(any())
}));
var rssOptionsValidator = object({
	title: string(),
	description: string(),
	...
	items: array(rssSchema).or(globResultValidator).transform((items) => {
		if (!Array.isArray(items)) {
			console.warn(s.yellow("[RSS] Passing a glob result directly has been deprecated. Please migrate to the `pagesGlobToRssItems()` helper: https://docs.astro.build/en/recipes/rss/"));
...
async function getRssResponse(rssOptions) {
	...
	return new Response(rssString, { headers: { "Content-Type": "application/xml" } });
}
async function getRssString(rssOptions) {
	return await generateRSS(await validateRssOptions(rssOptions));
}
async function validateRssOptions(rssOptions) {
	...
	throw new Error([`[RSS] Invalid or missing options:`, ...parsedResult.error.issues.map((zodError) => {
		const path = zodError.path.join(".");
		const message = `${zodError.message} (${path})`;
		const code = zodError.code;
		if (path === "items" && code === "invalid_union") return [
			message,
...
function pagesGlobToRssItems(items) {
	return Promise.all(Object.entries(items).map(async ([filePath, getInfo]) => {
		const { url, frontmatter } = await getInfo();
		if (url === void 0 || url === null) throw new Error(`[RSS] You can only glob entries within 'src/pages/' when passing import.meta.glob() directly. Consider mapping the result to an array of RSSFeedItems. See the RSS docs for usage examples: https://docs.astro.build/en/recipes/rss/`);
		...
		if (parsedResult.success) return parsedResult.data;
		const formattedError = new Error([`[RSS] ${filePath} has invalid or missing frontmatter.
Fix the following properties:`, ...parsedResult.error.issues.map((zodError) => zodError.message)].join("\n"));
...
async function generateRSS(rssOptions) {
	...
	if (items.find((result) => result.content)) {
		...
		if (rssOptions.xmlns?.content && rssOptions.xmlns.content === XMLContentNamespace) delete rssOptions.xmlns.content;
	}
	if (rssOptions.xmlns) for (const [k, v] of Object.entries(rssOptions.xmlns)) root.rss[`@_xmlns:${k}`] = v;
	root.rss.channel = {
		title: rssOptions.title,
		description: rssOptions.description,
		link: createCanonicalURL(site, rssOptions.trailingSlash, void 0)
	};
	if (typeof rssOptions.customData === "string") Object.assign(root.rss.channel, parser.parse(`<channel>${rssOptions.customData}</channel>`).channel);
	root.rss.channel.item = items.map((result) => {
		...
		if (result.description) item.description = result.description;
		if (result.pubDate) item.pubDate = result.pubDate.toUTCString();
		if (typeof result.content === "string") item["content:encoded"] = result.content;
		if (typeof result.customData === "string") Object.assign(item, parser.parse(`<item>${result.customData}</item>`).item);
		if (Array.isArray(result.categories)) item.category = result.categories;
		if (typeof result.author === "string") item.author = result.author;
		if (typeof result.commentsUrl === "string") item.comments = isValidURL(result.commentsUrl) ? result.commentsUrl : createCanonicalURL(result.commentsUrl, rssOptions.trailingSlash, site);
		if (result.source) item.source = {
			"#text": result.source.title,
			"@_url": result.source.url
		};
		if (result.enclosure) item.enclosure = {
			"@_url": isValidURL(result.enclosure.url) ? result.enclosure.url : createCanonicalURL(result.enclosure.url, rssOptions.trailingSlash, site),
			"@_length": result.enclosure.length,
			"@_type": result.enclosure.type
		};
		return item;
	});
	return new json2xml_default(xmlOptions).build(root);
}
//#endregion
//#region src/pages/rss.xml.ts
var rss_xml_exports = /* @__PURE__ */ __exportAll({ GET: () => GET });
async function GET(context) {
	const posts = await getD1Posts(context);
